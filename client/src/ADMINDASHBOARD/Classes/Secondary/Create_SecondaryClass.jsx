...
function Create_PrimaryClass() {
  ...
  useEffect(() => {
    ...
    axios.get('/api/v1/adminRoute/getAllClass', {withCredentials: true})
      .then((response) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true)
      const response = axios.post('/api/v1/adminRoute/createClass', formData, {
  ...
  const handleDelete = async (className) => {
    try {
      const re = await axios.get(`/api/v1/adminRoute/getAllClass?className=${className}`, {withCredentials: true});

      // Make an API request to delete the row from the server
      const response = await axios.delete(`/api/v1/adminRoute/deleteClass?_id=${re.data.classList[0]._id}`, { withCredentials: true });

...
const Attendance = () => {
  ...
  useEffect(() => {
    const fetchStudentData = async () => {
      try {
        const response = await axios.get("/api/v1/adminRoute/getAllStudents", {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        "api/v1/teacher/createAttendance",
        {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get("api/v1/teacher/getAttendance", {
        params: {
          year: year,
          month: month,
        },

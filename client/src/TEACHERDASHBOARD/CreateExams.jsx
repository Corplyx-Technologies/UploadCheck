...
const CreateExams = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`api/v1/exam/getAllExams?className=${data.classTeacher}&section=${data.section}`, {
  ...
  const handleDelete= async (id)=>{
    ...
    try {
      const response = await axios.delete(`api/v1/exam/deleteExam/${id}`, {
        withCredentials: true
      });
  ...
  const handleSubmit = async () => {
    try {
      ...
      await axios.post("api/v1/exam/createExam", formData, {
        withCredentials: true,
        headers: {
